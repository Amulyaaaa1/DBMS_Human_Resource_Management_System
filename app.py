...
def filter_salary_grade():
    ...
    # Query to get filtered employee details
    query = """
    ...
    # Query to get the total number of employees in the selected grade
    count_query = """
    ...
    # Query to get the maximum salary in the selected grade
    max_salary_query = """
    ...
    # Query to get the maximum bonus in the selected grade
    max_bonus_query = """
        SELECT MAX(s.sal_bonus) as max_bonus
        FROM Salary s
        WHERE s.salgrade = %s
    """
    
    # Query to get the minimum salary in the selected grade
    min_salary_query = """
    ...
    # Query to get the minimum bonus in the selected grade
    min_bonus_query = """
        SELECT MIN(s.sal_bonus) as min_bonus
        FROM Salary s
        WHERE s.salgrade = %s
    """
    ...
    total_employees = execute_query(count_query, (grade,), fetchone=True)['total_employees']
    max_salary = execute_query(max_salary_query, (grade,), fetchone=True)['max_salary']
    max_bonus = execute_query(max_bonus_query, (grade,), fetchone=True)['max_bonus']
    min_salary = execute_query(min_salary_query, (grade,), fetchone=True)['min_salary']
    min_bonus = execute_query(min_bonus_query, (grade,), fetchone=True)['min_bonus']
...
def insert_employee():
    ...
    # Try to get the last inserted empid, but handle errors
    try:
        empid = get_last_inserted_empid()
    except Exception as e:
        empid = None
        print(f"Error occurred while fetching last inserted empid: {e}")  # Log the error for debugging

    # If empid is not None, insert achievements
    if empid:
    ...
    # Redirect to the employee info page (even if empid retrieval failed)
...
def get_last_inserted_empid():
    ...
    # Check if the result is a valid tuple/list and contains at least one item
    if result and len(result) > 0:
        return result[0]
    else:
        # Handle the case where no result was returned (e.g., insert failed)
        return None
...
def add_emp_record():
    ...
    if medical_leave > 30:
        ...
        flash("Vacation cannot exceed 15.", 'error')
        return redirect(url_for('get_emp_records'))
...
def login():
    if request.method == 'POST':
        ...
        try:
            ...
            if user:
                # Get the user's role from the database record
                role = user.get('role')
